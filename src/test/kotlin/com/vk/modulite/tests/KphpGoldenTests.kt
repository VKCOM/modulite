...
class KphpGoldenTests : ModuliteInspectionTestBase() {
    ...
    fun `test 100_wrong_static_bindings`() {
        val inputStream =
            javaClass.classLoader.getResourceAsStream("KphpGoldenTests/100_wrong_static_bindings_ErrorExpected.txt")
        val expectedMessage = inputStream?.bufferedReader()?.readText()

        return UsefulTestCase.assertThrows(
            AssertionError::class.java, expectedMessage
        ) { runFixture("kphp_golden/100_wrong_static_bindings") }
    }
}
